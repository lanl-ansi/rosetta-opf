...
@variable(model, jmp_va[i in keys(ref[:bus])])
@variable(model, ref[:bus][i]["vmin"] <= jmp_vm[i in keys(ref[:bus])] <= ref[:bus][i]["vmax"], start=1.0)

@variable(model, ref[:gen][i]["pmin"] <= jmp_pg[i in keys(ref[:gen])] <= ref[:gen][i]["pmax"])
@variable(model, ref[:gen][i]["qmin"] <= jmp_qg[i in keys(ref[:gen])] <= ref[:gen][i]["qmax"])

@variable(model, -ref[:branch][l]["rate_a"] <= jmp_p[(l,i,j) in ref[:arcs]] <= ref[:branch][l]["rate_a"])
@variable(model, -ref[:branch][l]["rate_a"] <= jmp_q[(l,i,j) in ref[:arcs]] <= ref[:branch][l]["rate_a"])

@objective(model, Min, sum(gen["cost"][1]*jmp_pg[i]^2 + gen["cost"][2]*jmp_pg[i] + gen["cost"][3] for (i,gen) in ref[:gen]))

for (i,bus) in ref[:ref_buses]
    @constraint(model, jmp_va[i] == 0)
end
...
for (i,bus) in ref[:bus]
    ...
    @constraint(model,
        sum(jmp_p[a] for a in ref[:bus_arcs][i]) ==
        sum(jmp_pg[g] for g in ref[:bus_gens][i]) -
        sum(load["pd"] for load in bus_loads) -
        sum(shunt["gs"] for shunt in bus_shunts)*jmp_vm[i]^2
    )

    @constraint(model,
        sum(jmp_q[a] for a in ref[:bus_arcs][i]) ==
        sum(jmp_qg[g] for g in ref[:bus_gens][i]) -
        sum(load["qd"] for load in bus_loads) +
        sum(shunt["bs"] for shunt in bus_shunts)*jmp_vm[i]^2
    )
end
...
for (i,branch) in ref[:branch]
    ...
    p_fr = jmp_p[f_idx]                     # p_fr is a reference to the optimization variable p[f_idx]
    q_fr = jmp_q[f_idx]                     # q_fr is a reference to the optimization variable q[f_idx]
    p_to = jmp_p[t_idx]                     # p_to is a reference to the optimization variable p[t_idx]
    q_to = jmp_q[t_idx]                     # q_to is a reference to the optimization variable q[t_idx]

    vm_fr = jmp_vm[branch["f_bus"]]         # vm_fr is a reference to the optimization variable vm on the from side of the branch
    vm_to = jmp_vm[branch["t_bus"]]         # vm_to is a reference to the optimization variable vm on the to side of the branch
    va_fr = jmp_va[branch["f_bus"]]         # va_fr is a reference to the optimization variable va on the from side of the branch
    va_to = jmp_va[branch["t_bus"]]         # va_fr is a reference to the optimization variable va on the to side of the branch
...
for (i,gen) in ref[:gen]
    ...
    push!(var_init, value(jmp_pg[i])) # non-standard start
    ...
    push!(var_init, value(jmp_qg[i])) # non-standard start
...
for (l,i,j) in ref[:arcs]
    ...
    #push!(var_init, 0.0) #p
    push!(var_init, value(jmp_p[(l,i,j)]))
    ...
    #push!(var_init, 0.0) #q
    push!(var_init, value(jmp_q[(l,i,j)]))
    push!(var_lb, -branch["rate_a"])
    push!(var_ub,  branch["rate_a"])
